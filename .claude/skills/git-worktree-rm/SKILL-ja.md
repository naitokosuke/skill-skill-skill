...
- `gwq remove <pattern>` で worktree を削除
- `gwq remove -b <branch>` で worktree とブランチを同時削除
- `gwq remove --dry-run <pattern>` でプレビュー
- `gwq remove -f <pattern>` で未コミット変更があっても強制削除（dirty worktree 用）
- `gwq remove --force-delete-branch <branch>` でブランチが未マージでも削除（`-b` 併用時）

`-f` は worktree 側の強制（dirty な変更を無視）、`--force-delete-branch` はブランチ側の強制（未マージ拒否を無視）で、目的が異なる

## dry-run の使い方

プレビューは実削除で打つ予定のフラグ構成をそのまま `--dry-run` 付きで打つ
`-b` を付けるかどうかで「ブランチも消えるか」の見え方が変わるので忠実に揃える
