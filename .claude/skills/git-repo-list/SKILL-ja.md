...
- `ghq list` で相対パスで一覧
- `ghq list -p` で絶対パスで一覧
- `ghq list -e <name>` で完全一致検索
- `ghq list <pattern>` で部分一致検索
- `ghq root` でプライマリ root を表示
- `ghq root --all` ですべての root を表示
...
`ghq` が未インストールの場合はその旨をユーザーに伝える
代替手段を求められた場合は `find` / `fd` で `~/ghq` や `~/src` などクローン配置の慣習パスを走査することになるが、精度は落ちる
